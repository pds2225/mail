...
def compute_match_score(item: dict[str, Any], company: dict[str, Any]) -> dict[str, Any]:
    ...
    if district and district in text:
        ...
        # 기업 지역이 본문에 없고, 다른 특정 광역만 언급되면 mismatch
        other_regions = [r for r in KNOWN_REGIONS if r in text and r != city]
        company_region_mentioned = (city and city in text) or (district and district in text)
        if other_regions and not company_region_mentioned:
