...
| 단계 | 상태 | 운영자가 체감하는 것 |
|------|------|----------------------|
| **준비 (W0)** | ✅ 완료 | 시스템이 “정상 / 주의 / 발송보류”를 **구분해서 기록**할 수 있게 됨. 큰 포털·작은 지역사이트를 **다른 민감도**로 둘 설정 파일도 준비됨. 아직은 “표시·기록” 중심이고, 메일을 실제로 멈추는 스위치는 다음 단계에서 연결 |
| **사이트별 민감도 (W1)** | 🔄 이번 착수 | 같은 “오늘 0건”이라도 **기업마당은 긴급**, **연간·지역 사이트는 경고만** 나오도록 실제로 적용 |
| **자동 재시도·확인 목록 (W2)** | 대기 | 이상 사이트 자동 한두 번 다시 가져오기 + 사람이 볼 대기열 |
| **상세 읽기 구분 (W3)** | 대기 | “원문에 없음 / 읽기 실패 / 접속 실패”를 화면에 다르게 표시 |

**한 줄:** 지금은 “이상을 **정확히 이름 붙여 기록**”하는 단계까지 끝났고, 다음은 “사이트마다 **덜 예민/더 예민**하게 알림”을 켜는 단계입니다.
