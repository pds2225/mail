...
기존 인프라를 버리지 않는다. `docs/project/TASKS.md` / `docs/project/RULES.md` / `scripts/auto_dev_queue.py` /
...
│  → 제품 비전 / config/groups.json 방향 / “무엇을 좋은 추천으로 볼지” 갱신     │
└────────────────────────────┬────────────────────────────────────┘
                             │ 명세·평가 기준 갱신
┌────────────────────────────▼────────────────────────────────────┐
│ L2 개발자(운영자) 피드백 루프 (수십 분~수 시간)                       │
│  accuracy matrix · FP/FN 클러스터 · 골든라벨 의심분 · BLOCKED 큐      │
│  → docs/project/TASKS.md / eval rubric / 허용 수정 범위 갱신                      │
└────────────────────────────┬────────────────────────────────────┘
                             │ 코딩 명세 + 게이트
┌────────────────────────────▼────────────────────────────────────┐
...
| **L2** | 운영자 + accuracy harness | `docs/project/TASKS.md`, 루브릭, 골든라벨 | **배치 승인 1회** (빈틈 고칠까?) |
| **L3** | 고객·수신자·사이트 현실 | `config/groups.json` 방향, intake | **비전 판단만** |
...
| **메모리/상태** | 다음에 남길 것 | `var/state/auto_dev_state.json`, `docs/project/TASKS.md`, `done/failed/blocked_tasks.md`, matrix·라벨 |
...
| **G2** 사람 리뷰 opt-out (`needs-human`/`blocked`/Draft) | 사람이 멈추고 싶을 때만 |
...
| 자산 | 경로 | 역할 | 드리프트 신호 |
|------|------|------|----------------|
| 안전규칙 | `docs/project/RULES.md` | 발송·보호파일·재시도 | 규칙과 코드 불일치 |
| 태스크 큐 | `docs/project/TASKS.md` | L2→L1 명세 | PENDING 적체, RUNNING 고아 |
...
| 실행 상태 | `var/state/auto_dev_state.json` | 메모리 | retry_counts 폭주 |
...
| 상태 | docs/project/failed_tasks.md + FIX TASK id |
