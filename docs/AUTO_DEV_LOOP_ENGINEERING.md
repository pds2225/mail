...
flowchart TB
  subgraph L3["L3 외부 피드백 루프 (일~주)"]
    ...
    T[docs/project/TASKS.md 백로그 갱신]
    M --> T
  end
...
| 메모리 | `var/state/auto_dev_state.json`, `docs/project/done_tasks.md` | 재시도 횟수, 마지막 게이트 결과 |
...
| UI `/sites/add` 패킷 | `docs/works/SITE_ADD_PR_PACKET.md` | 사이트 추가 TASK |

L3는 **메트릭·알림이 docs/project/TASKS.md에 항목을 추가**하고, L1이 소비한다. 사람은 백로그 우선순위만 가끔 조정한다.
...
| **메모리** | state + work assets 버전 | `var/state/auto_dev_state.json`, `work_assets.json` checksum |
...
```
Tier 0 — 안전 (항상)
  ├─ preflight: docs/project/TASKS.md 구조, docs/project/RULES.md 존재
...
| 자산 | 경로 | 드리프트 징후 | 관리 루프 |
|------|------|---------------|-----------|
| 안전 헌법 | `docs/project/RULES.md` | 발송/Secret 규칙 누락 | 분기 1회 감사 TASK |
| 큐 백로그 | `docs/project/TASKS.md` | RUNNING 고착, PENDING 비대 | `stale_running_hours` 알림 |
| 에이전트 컨텍스트 | `AGENTS.md` | 테스트 명령 불일치 | CI preflight |
| 검증 스크립트 | `scripts/*_gate.py` | 게이트 우회 PR | diff 시 필수 실행 |
| PR 패킷 템플릿 | `docs/works/*.md` | 체크리스트 미갱신 | UI 생성 시 덮어쓰기 |
...
docs/project/TASKS.md / docs/project/RULES.md / var/state/auto_dev_state.json   # 기존
```
...
5. docs/project/TASKS.md / var/state/auto_dev_state.json 커밋 ([skip ci])
```
...
1. `docs/project/TASKS.md` 항목 + `docs/project/RULES.md` + `AGENTS.md` + 관련 `docs/works/` 패킷을 **단일 루프 프롬프트**로 조립
...
- `config/sites.json`, `수신자` → `config_data`
...
| `docs/works/*_PACKET.md` | L1 실행 컨텍스트 자산 |
| `coverage_alert.py` | L3 → docs/project/TASKS.md 피더 |
...
- [ ] L3: `coverage_alert` → docs/project/TASKS.md 자동 enqueue
...
**매주 (사람 5분):** `docs/project/blocked_tasks.md`, RUNNING 고착, `loop_drift_check` 리포트

**매월:** `work_assets.json` checksum 갱신, docs/project/RULES.md 감사 TASK 실행
