...
- [~] HMAC 토큰에 issued_at을 포함하고 TTL 만료를 강제한다.
- [~] timestamp 없는 legacy 16-hex 토큰은 기본 거부한다.
- [~] 과도한 미래 토큰도 clock-skew 범위 밖이면 거부한다.
- [~] 기존 mailto/IMAP read-only 흐름은 유지한다.
- [ ] CI 회귀 통과 후 Risk 132 crosswalk를 ALREADY_DONE으로 갱신한다.
