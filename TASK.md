...
- STATUS: IN_PROGRESS
- REQUEST_SOLVED=NO
- CHECKPOINT: P0_EVIDENCE_PASS_V3 — 154/154 owner 검증, ALREADY_DONE=26, COVERED_BY_TASK=128, OPEN_WITHOUT_TASK=0
- P0: 전체 27, ALREADY_DONE=14, 미해결/진행=13
- VERIFIED_DONE_RISK: 12, 16, 17, 18, 19, 20, 32, 33, 39, 46, 47, 59, 67, 96, 103, 113, 114, 115, 116, 119, 120, 126, 129, 143, 147, 149
- NEXT: 실제 P0 gap 최소수정. Risk 132 token expiry PR과 27/28/97/99/101/102/118/138/142/144/152/153 순으로 dependency 확인 후 진행
...
- TASK_ID: MAIL-024
- TASK_START_SHA: dbed97c57d103e72f7d525170e0d2898e44a1f4a (origin/main)
- WORK_BRANCH: task/mail-024-risk1-20-audit-part2
- CHECKPOINT: DATA_AUDIT 완료(20/20 근거 상태 확정). 코드 갭 0건 — 전부 기존 설계로 커버됨.

### MUST
- [x] 1~20 각각 코드/테스트/재현 근거 확인 (20/20 완료)
- [x] 이미 구현된 항목 재개발 금지 — 감사만 하고 새 코드 작성 안 함(모두 이미 존재하는 안전장치로 확인됨)
- [x] HTTP 200 오판, 구조/API 변경, pagination, rate limit, 차단, 인증, timeout, 장애격리, run lock, 급감/급증, SSRF 점검
- [x] 원인별 최소 변경 + 회귀테스트 (실제 코드 갭 0건 발견 — 추측성 신규 코드는 작성하지 않음)
...
**제네릭 안전망으로만 커버(전용 감지·테스트는 없음, COVERED_BY_TASK 유지 — 재현되는 인시던트 근거가 없어 추측성 신규 코드는 작성하지 않음):**

| Risk | 문제 | 현재 커버 방식 | 안 되는 것 |
|---:|---|---|---|
| 2 | 사이트 HTML 구조 변경 | `fetch_all` 예외 격리 + `coverage_alert` 급감(0건) 탐지 + `notice_validity`의 title/link 둘 다 없으면 parse_error | 구조가 바뀌어도 title/link 둘 다는 남고 다른 필드만 틀어지는 "조용히 틀린 값" 케이스는 못 잡음 |
| 3 | API 스키마 변경 | 위와 동일 | 위와 동일(JSON 필드명이 바뀌어도 예외 없이 빈 문자열로 넘어가면 조용히 틀림) |
| 6 | API 호출 한도 초과 | `max_pages`/`api_max_pages`로 소스별 요청량 자체를 상한 이하로 제한(`tests/test_core_sources_specialize.py`가 설정값 존재는 확인). `_http_get`/`_soup`는 4xx/5xx를 의도적으로 재시도하지 않음(주석: "페이지 수준 오류라 재시도가 무의미") | 429 전용 Retry-After 백오프·재시도는 없음. 실제 429 발생 재현 근거 없음 |
| 7 | 사이트별 차단정책 | 실제 브라우저와 동일한 `User-Agent`/`Accept`/`Sec-Fetch-*` 헤더 세트, 3단계 SSL 폴백 | 403/차단 응답을 "다른 실패"와 구분해 별도 알림하는 로직 없음(예: 인증키 만료와 동일하게 `HTTPStatusError`로만 처리) |
| 9 | 인증키·쿠키 만료 | 401/403도 `fetch_all`/coverage_alert의 generic 실패 격리·급감탐지로 잡힘(그 소스가 조용히 0건이 되지 않음) | 401을 "키 만료"로 특정해 알리는 전용 로직 없음 |
| 11 | 한글 인코딩 오류 | httpx의 Content-Type charset 자동 감지 + BeautifulSoup 파싱에 의존 | 전용 EUC-KR/CP949 폴백이나 깨짐 감지 테스트 없음 |
| 13 | 소스가 오래된 캐시 제공 | 없음 — `HTTP_HEADERS`에 `Cache-Control: no-cache` 미포함 | 전용 캐시버스팅 없음. 단, 실제 stale-cache 인시던트 재현 근거도 없음 |

이 7개는 "안 만들면 안 되는 상태"가 아니라 "더 튼튼하게 만들 수는 있지만 지금 그렇게 할 근거(재현되는 실제 사고)가 없는" 상태다. 1~2건의 가상 시나리오만으로 새 코드를 추가하지 않는다는 전역 원칙(§8-8 FORBIDDEN)에 따라 이번 패스에서는 코드를 추가하지 않았다. 실제 인시던트(예: 특정 소스가 며칠간 옛 목록만 주거나, 401이 반복돼도 아무도 모르는 사고)가 재현되면 그때 원인별 최소 수정을 한다.

### DONE
REQUEST_SOLVED=YES — Risk 1~20 전체 20개의 근거 상태를 확정했다. 13개는 코드+통과하는 테스트로
ALREADY_DONE 확정(crosswalk JSON 승격 완료), 7개는 제네릭 안전망(소스별 실패 격리 + 수집량 이상탐지)
으로 커버되지만 전용 감지는 없음 — 재현 가능한 실제 인시던트 근거가 없어 추측성 신규 코드는 작성하지
않고 COVERED_BY_TASK로 유지한다. 이번 감사에서 실제 코드 갭은 0건이었다(모두 기존 설계로 이미
커버됨). 실제 인시던트가 재현되면 그때 원인별 최소 수정 + 회귀테스트를 추가한다.
