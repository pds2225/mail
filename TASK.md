...
[~] MAIL-P0C-01 | 최근 3영업일 재조회와 중요 변경을 안전하게 복구한다
...
> 최근 3영업일 재조회 기능을 완성한다. 단순 72시간이 아닌 영업일 기준으로 금요일→월요일, 주말 제외, 확장 가능한 공휴일, 수정공고·재공고·마감연장·신청대상·지역·지원금·접수 URL 변경, 중요 변경 감지, 동일 버전 중복 발송 방지, 기존 seen/delivery 상태 충돌 방지를 지원한다. 기업 매칭으로 Hard Gate 탈락을 부활시키지 않는다.

### 8-2. 비개발자용 1줄 요약

최근 3영업일 공고의 수정·재공고를 다시 찾아 실제 중요 변경만 안전하게 재판정한다
...
- STATUS: ACTIVE
- DEPENDS_ON: 없음
- WORK_BRANCH: feat/MAIL-P0C-01-recent-3-business-days
- TASK_START_SHA: fe8a54bcd0fc7ce128a72baf85b99fc0e4aa31c4
- TASK_BLOB_SHA: 11688a6f3bb9c85d7e3e8e001463f17a789296ab
- 실제 메일 발송·삭제·대량 라벨 변경은 하지 않는다.

### 8-4. MUST — 반드시 구현

- [ ] 단순 72시간이 아닌 영업일 기준 재조회 창을 구현한다.
- [ ] 금요일→월요일과 주말 제외를 검증한다.
- [ ] 공휴일은 확장 가능한 목록/주입 구조로 처리한다.
- [ ] 수정공고·재공고·마감연장·신청대상·지역·지원금·접수 URL 변경을 감지한다.
- [ ] 과거 공고의 중요 변경만 재판정하고 동일 버전은 중복 발송하지 않는다.
- [ ] 기존 seen/delivery 상태와 충돌하지 않게 멱등성을 유지한다.
- [ ] 기업 매칭으로 Hard Gate 탈락 공고를 부활시키지 않는다.

### 8-5. KEEP / FORBIDDEN

- 기존 수집·판정·중복제거·delivery 구조와 dry-run 기본값을 유지한다.
- .env, Secret, 고객정보, 메일 원문을 출력·커밋하지 않는다.
- 실제 이메일 발송·삭제·대량 라벨 변경을 하지 않는다.
- main 직접 개발·push를 하지 않고 전용 PR 경로만 사용한다.
- 현재 TASK에 필수가 아닌 결함·개선사항은 기록만 하고 자동 개발하지 않는다.

### 8-6. VERIFY

- [ ] MAIL-P0C-01 focused 회귀 테스트
- [ ] python -m compileall .
- [ ] python -m pytest
- [ ] git diff --check
- [ ] YAML/JSON parser 검증
- [ ] dry-run에서 메일 발송·삭제·대량 라벨 변경 0건

### 8-7. DONE

REQUEST_SOLVED: NO — 구현·검증·PR·조건부 자동머지 후에만 YES로 갱신한다.
