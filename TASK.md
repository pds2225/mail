...
- 이번 작업은 분석. 매칭 정책 수정은 별도 요청 전까지 하지 않음
...
PINNING:
- TASK_ID: MAIL-003
- TASK_START_SHA: origin/main 55f3251f
- WORK_BRANCH: cursor/prestartup-fit-analysis-e94f

런타임 재현 (실발송 없음, `evaluate_notice` + `score_and_filter`):
...
원인: 1차는 AND(AI+사업화 등)로 넓게 통과, 2차는 OR 문구(`AI 솔루션` 등)만 점수에 넣어 예비창업 본공고는 떨어지고 기창업 솔루션 공고는 남음. `business_years` 없음. LLM 밴드 40~70인데 실제 점수는 0 또는 5라 2차 LLM이 안 돈다.
