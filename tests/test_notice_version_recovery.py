...
def test_load_settings_default_days_back_matches_runtime_fallback(tmp_path, monkeypatch):
    """settings.json 에 days_back 키 자체가 없을 때도 load_settings() 기본값이
    execute_monitor() 의 실제 런타임 폴백(3)과 일치해야 한다(MAIL-P0C-01 회귀).
    이전엔 load_settings() 기본값이 1이라, 키가 사라지면 조용히 최근 3영업일이
    아닌 1일로 좁아질 수 있었다."""
    ...
    assert m.load_settings()["days_back"] == 3
