...
def test_ema_selector_excludes_attachment_icon_from_title():
    """EMA 제목 링크 안의 아이콘 텍스트 'file'을 공고 제목으로 합치지 않는다."""
    root = pathlib.Path(__file__).parent.parent
    sites = json.loads((root / "sites.json").read_text(encoding="utf-8"))
    site = next(s for s in sites if s["id"] == "ema")
    html = """
    <table><tbody><tr>
      <td class="subject"><a href="/bbs/read/1">
        <span>2026년도 AI 사업 수혜기업 모집 공고</span>
        <i class="sp-ico file">file</i>
      </a></td>
      <td class="date">2026-07-23</td>
    </tr></tbody></table>
    """
    soup = monitor.BeautifulSoup(html, "html.parser")
    items = monitor._generic_page_items(soup, site, site["url"])

    assert len(items) == 1
    assert items[0]["title"] == "2026년도 AI 사업 수혜기업 모집 공고"
    assert items[0]["posted_date"] == "2026-07-23"
