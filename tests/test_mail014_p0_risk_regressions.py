...
These tests only verify existing safety contracts; they do not send email or access the network.
"""
...
def test_send_to_list_builds_one_recipient_per_message(monkeypatch):
    ...
    def fake_send(subject, body, to):
        built.append(m._build_mime_message(subject, body, to))
    ...
    assert len(built) == 2
    assert [msg["To"] for msg in built] == recipients
    assert all(msg.get("Cc") is None for msg in built)
    assert all("," not in str(msg["To"]) for msg in built)
